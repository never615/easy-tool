    ...
    private function seedNewConfigDefinitions(): void
    {
        $definitions = [
            [
                ...
                'default_value' => '0',
                'options' => '0,1',
                'remark' => '默认关闭。保存后发布运行期 env 并刷新 config cache，手动 reload 后新 worker 生效。',
            ...
            [
                ...
                'remark' => 'summary 只记录聚合指标；trace 记录逐条等待和运行中明细；off 关闭。手动 reload 后生效。',
                'requires_reload' => true,
                'sort' => 110,
            ],
            [
                ...
                'remark' => '取值 0 到 1。summary 模式下可临时设置 0.01 追踪约 1% task。手动 reload 后生效。',
                'requires_reload' => true,
                'sort' => 120,
            ],
        ];
