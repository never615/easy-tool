...
class NewConfigValuesFile
{
    ...
    private function atomicWrite(string $filepath, string $content): void
    {
        ...
        if (!is_dir($directory)) {
            throw new RuntimeException("Runtime values directory does not exist: {$directory}");
        }

        $tmpPath = $directory . '/.' . basename($filepath) . '.' . getmypid() . '.tmp';
        if (file_put_contents($tmpPath, $content, LOCK_EX) === false) {
            throw new RuntimeException("Failed to write temporary runtime values file: {$tmpPath}");
        }
        ...
        if (!rename($tmpPath, $filepath)) {
            @unlink($tmpPath);
            throw new RuntimeException("Failed to replace runtime values file: {$filepath}");
        }
